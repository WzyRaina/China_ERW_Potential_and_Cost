# 计算每年铺洒 20 t rock/ha，直到达到 150 t rock/ha 的上限/达到Ni红线停止铺洒，然后计算年平均风化速率
...
print("[INFO] Loading Ni Constraint Maps...")
Ni_limit_basalt_array = gdal.Open('./Output/EW/China/Discussion/Ni/Max_Basalt_Limit_tha.tif').ReadAsArray()
Ni_limit_olivine_array = gdal.Open('./Output/EW/China/Discussion/Ni/Max_Olivine_Limit_tha.tif').ReadAsArray()
...
for r in range(num_of_type):  # num_of_type
    ...
    if rock_name.lower() == 'basalt':
        current_Ni_limit_array = Ni_limit_basalt_array
    elif rock_name.lower() in ['dunite', 'olivine']:
        current_Ni_limit_array = Ni_limit_olivine_array
    else:
        current_Ni_limit_array = np.full((raster_row, raster_col), np.nan)
    ...
    for s in range(num_of_grain_size):  # num_of_grain_size
        ...
        for row in range(raster_row):  # row, 360, raster_row
                            ...
                            if EEF_array[row, col] >= 0:
                                if PH_array[row, col] > 0:
                                    if soil_temperature_array[row, col] > -256:
                                        ...
                                        # 获取当前网格的Ni累积施加约束上限
                                        Ni_limit_val = current_Ni_limit_array[row, col]
                                        # 如果因为缺失值导致提取到 NaN 或者红线被判定为0以下，则视为不允许铺洒
                                        if np.isnan(Ni_limit_val) or Ni_limit_val <= 0:
                                            Ni_limit_val = 0
                                        ...
                                        for yr in range(end_year - start_year + 1):  # 在2025到2200年期间
                                            if yr == 0:  # 第一年铺洒逻辑
                                                ...
                                                # 约束1: 物理留存量没超标 (<= 150)
                                                cond_residual_ok = (rock_volume[yr - 1] - weathering_volume[yr] <= 150)
                                                # 约束2: 加上今年的铺洒量，历史总累积量不超标 (<= Ni_limit_val)
                                                cond_cumulative_ok = ((cumulative_applied + EW_data.iloc[15, 2]) <= Ni_limit_val)

                                                if cond_residual_ok and cond_cumulative_ok:
                                                # 必须同时满足，才能执行铺洒
                                                    apply_years[yr] = 1
                                                    apply_years_value += 1
                                                    rock_volume[yr] = rock_volume[yr - 1] + EW_data.iloc[15, 2] - weathering_volume[yr]  # 每年农田的岩石数量是在前一年的基础上，新增20t rock，再减去之前几年铺洒岩石的风化量
                                                    cumulative_applied += EW_data.iloc[15, 2]
                                                ...
                                                else:  # 如果物理上限超标，或重金属红线超标，皆不再铺洒，只风化
                                                    apply_years[yr] = 0
                                                    apply_years_value += 0
        ...
        EW_potential_raster_rock_rem = driver.Create(f'./Output/EW/China/Remove Potential/EW_multi_year_remove_potential_raster_{rock_name}_{current_p80} μm_Ni_Constrained_1.tif', xsize=raster_col, ysize=raster_row, bands=1, eType=gdal.GDT_Float32)  # note: xsize=ecological_zones_raster_array.shape[1], not [0]
...
EW_rem_potential_csv_df.to_csv('./Output/EW/China/Remove Potential/Multi Year Remove Potential_Ni_Constrained_1.csv')
