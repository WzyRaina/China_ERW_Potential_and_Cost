...
def compute_removal_cost(
    ...
    # 记录每个网格的成本
    # 动态计算情景总数 (比如 2 种岩石 * 11 种粒径 = 22)
    num_tech = num_of_type * num_of_grain_size
    # 使用 raster_row * raster_col 替代 360*720，并使用动态的 num_tech
    EW_rem_cost_csv = np.full((raster_row * raster_col, num_tech), np.nan)
    ...
    for r in range(num_of_type):
        ...
        for s in range(num_of_grain_size):
            ...
            # 提取当前粒径并转为整型，防止产生 "100.0 μm" 找不到文件的错误
            current_p80 = int(grain_size.iloc[s, 0])
            rock_name = rock_type.iloc[r, 0]

            if current_p80 <= 50:
                # 针对 2, 10, 20, 50 μm：采用加水铺洒 (Liquid - spraying)，引入传入的溢价系数 application_price_factor
                current_application_price = rock_application_price * application_price_factor
            else:
                # 针对 100, 250, 500 μm 及以上：采用原干撒成本 (Dry bulk)
                current_application_price = rock_application_price

            print(f"Calculating Cost for: {rock_name}, {current_p80} μm (App Price: ${current_application_price:.3f}/t)...")
            ...
            # CO2_seq_potential_rock = gdal.Open(f'E:/wzy/1-CDR-Base/EW/Output/Raster Data/2025/CO2_seq_potential_{rock_name}(tCO2·t rock-1)_{current_p80} μm_1.tif').ReadAsArray()  # dunite/basalt, mean
            # transport distance
                        ...
                        if EW_land_array[row, col] > 0:  # available for EW
                            if not np.isnan(EW_global_transport_distance_array_rock[row, col]):  # no rock source in this country
                                if EEF_array[row, col] >= 0:
                                    ...
                                    EW_cost_structure.iloc[3, 1] = current_application_price  # $/t rock
                                    ...
                                    if CO2_removal_potential_rock > 0:
                                        ...
                                        EW_rem_cost_csv[csv_row_idx, csv_col_idx] = EW_cost_raster_array_rock_rem[row, col]
                                    else:
            ...
            if uncertainty == False:
                if pathway == 1:
                    ...
                    EW_cost_raster_rock_rem = driver.Create(f'E:/wzy/2-Grid-based-MONET/Output/EW/China/Remove Cost/EW_remove_cost_raster_{rock_name}_{current_p80} μm.tif', xsize=raster_col, ysize=raster_row, bands=1, eType=gdal.GDT_Float32)  # note: xsize=ecological_zones_raster_array.shape[1], not [0]
                ...
                if pathway == 2:
                    ...
                    driver = gdal.GetDriverByName("GTiff")
                    # note: xsize=ecological_zones_raster_array.shape[1], not [0]; eType=gdal.GDT_Float32
                    EW_cost_raster_rock_rem = driver.Create(f'E:/wzy/2-Grid-based-MONET/Output/EW/China/Remove Cost/EW_remove_cost_raster_{rock_name}_{current_p80} μm_1.tif', xsize=raster_col, ysize=raster_row, bands=1, eType=gdal.GDT_Float32)  # note: xsize=ecological_zones_raster_array.shape[1], not [0]
                    EW_cost_raster_rock_rem.SetGeoTransform([-180, 0.5, 0, 90, 0, -0.5])  # 设置地理变换参数;原点；左上角坐标(-180, 90)，像元大小(0.5 x 0.5)，即0.5°分辨率
                    srs = osr.SpatialReference()  # 设置坐标参考系为 WGS 1984 (EPSG:4326)
                    srs.ImportFromEPSG(4326)  # WGS 1984 的 EPSG 编号是 4326
                    EW_cost_raster_rock_rem.SetProjection(srs.ExportToWkt())
    ...
    if uncertainty == False:
        if pathway == 1:
            ...
            EW_rem_cost_csv_df.to_csv('E:/wzy/2-Grid-based-MONET/Output/EW/China/Remove Cost/Remove Cost.csv', index=False)
        if pathway == 2:
            EW_rem_cost_csv_df = pd.DataFrame(EW_rem_cost_csv)
            EW_rem_cost_csv_df.to_csv('E:/wzy/2-Grid-based-MONET/Output/EW/China/Remove Cost/Remove Cost_1.csv', index=False)
    else:
...
if __name__ == "__main__":
    ...
    EW_land_array = gdal.Open('C./Global Data/EW/Input/China/China_Cropland_CACD_2021_0.5.tif').ReadAsArray()
