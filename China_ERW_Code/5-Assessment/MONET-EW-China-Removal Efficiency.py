...
def compute_removal_efficiency(
    ...
    # 记录每个网格的移除效率和风化速率
    EW_removal_efficiency_csv = np.full((360*720, 8), np.nan)
    EW_weathering_rate_csv = np.full((360*720, 8), np.nan)
    ...
    for r in range(num_of_type):
        ...
        for s in range(num_of_grain_size):
            if uncertainty:
                if pathway == 1:
                    CO2_seq_potential_rock = CO2_seq_potential_dict[(rock_type.iloc[r, 0].lower(), f"{grain_size.iloc[s, 0]}μm")]
                if pathway == 2:
                    CO2_seq_potential_rock = CO2_seq_potential_dict[(rock_type.iloc[r, 0].lower(), f"{grain_size.iloc[s, 0]}μm_1")]
            else:
                if pathway == 1:
                    CO2_seq_potential_rock = gdal.Open(f'C:/PyCharm/PycharmProjects/NETs/EW/Output/Raster Data/CO2_seq_potential_{rock_type.iloc[r, 0]}(tCO2·t rock-1)_{grain_size.iloc[s, 0]} μm.tif').ReadAsArray()  # dunite/basalt, mean, 1.7
                if pathway == 2:
                    CO2_seq_potential_rock = gdal.Open(f'C:/PyCharm/PycharmProjects/NETs/EW/Output/Raster Data/CO2_seq_potential_{rock_type.iloc[r, 0]}(tCO2·t rock-1)_{grain_size.iloc[s, 0]} μm_1.tif').ReadAsArray()  # dunite/basalt, mean, 1
            # transport distance
            EW_China_transport_distance_array_rock = gdal.Open(f'C:/PyCharm/PycharmProjects/Grid-based MONET/Global Data/EW/Output/EW_China_transport_distance_{rock_type.iloc[r, 0]}.tif').ReadAsArray()  # no trade
            ...
            weathering_rate_array = gdal.Open(f'C:/PyCharm/PycharmProjects/NETs/EW/Output/Raster Data/weathering_rate_{rock_type.iloc[r, 0]}_{grain_size.iloc[s, 0]} μm.tif').ReadAsArray()
                        ...
                        if EW_land_array[row, col] > 0:  # 耕地像素值代表该网格耕地比例，0-100
                            if not np.isnan(EW_China_transport_distance_array_rock[row, col]):  # no rock source in this country
                                if EEF_array[row, col] >= 0:
                                    ...
                                    # CR_EW = pd.read_csv(f'C:/PyCharm/PycharmProjects/NETs/EW/Output/Weathering Rate_{grain_size.iloc[s, 0]} μm_pH{PH.iloc[p, 0]}.csv')  # order: dunite, basalt; mean, low, up
                                    ...
                                    if CO2_removal_potential_rock > 0:
                                        ...
                                        EW_weathering_rate_csv[row * 720 + col, r * 4 + s] = weathering_rate_array[row, col]
                                    else:
                                        # print("1")
                                    ...
                                    EW_removal_efficiency_csv[row * 720 + col, r * 4 + s] = EW_efficiency_raster_array_rock[row, col]
            ...
            if uncertainty==False:
                if pathway == 1:
                    ...
                    EW_efficiency_raster_rock = driver.Create(f'C:/PyCharm/PycharmProjects/Grid-based MONET/Output/EW/China/Removal Efficiency/EW_removal_efficiency_raster_{rock_type.iloc[r, 0]}_{grain_size.iloc[s, 0]} μm.tif', xsize=raster_col, ysize=raster_row, bands=1, eType=gdal.GDT_Float32)  # note: xsize=ecological_zones_raster_array.shape[1], not [0]
                ...
                if pathway == 2:
                    ...
                    # note: xsize=ecological_zones_raster_array.shape[1], not [0]; eType=gdal.GDT_Float32
                    # EW_efficiency_raster_rock = driver.Create(f'./Output/EW/Removal Efficiency/Global Output/EW_removal_efficiency_raster_{rock_type.iloc[r, 0]}_{grain_size.iloc[s, 0]} μm.tif', xsize=raster_col, ysize=raster_row, bands=1, eType=gdal.GDT_Float32)  # note: xsize=ecological_zones_raster_array.shape[1], not [0]
                    EW_efficiency_raster_rock = driver.Create(f'C:/PyCharm/PycharmProjects/Grid-based MONET/Output/EW/China/Removal Efficiency/EW_removal_efficiency_raster_{rock_type.iloc[r, 0]}_{grain_size.iloc[s, 0]} μm_1.tif', xsize=raster_col, ysize=raster_row, bands=1, eType=gdal.GDT_Float32)  # note: xsize=ecological_zones_raster_array.shape[1], not [0]
                    EW_efficiency_raster_rock.SetGeoTransform([-180, 0.5, 0, 90, 0, -0.5])  # 设置地理变换参数;原点；左上角坐标(-180, 90)，像元大小(0.5 x 0.5)，即0.5°分辨率
                    srs = osr.SpatialReference()  # 设置坐标参考系为 WGS 1984 (EPSG:4326)
                    srs.ImportFromEPSG(4326)  # WGS 1984 的 EPSG 编号是 4326
                    EW_efficiency_raster_rock.SetProjection(srs.ExportToWkt())
    ...
    if uncertainty == False:
        if pathway == 1:
            ...
            EW_removal_efficiency_csv_df.to_csv('C:/PyCharm/PycharmProjects/Grid-based MONET/Output/EW/China/Removal Efficiency/Removal Efficiency.csv')
            ...
            EW_weathering_rate_csv_df.to_csv('C:/PyCharm/PycharmProjects/Grid-based MONET/Output/EW/China/Removal Efficiency/Weathering Rate.csv')
        if pathway == 2:
            # 保存为csv文件
            EW_removal_efficiency_csv_df = pd.DataFrame(EW_removal_efficiency_csv)
            # EW_removal_efficiency_csv_df.to_csv('./Output/EW/Removal Efficiency/Global Output/Removal Efficiency.csv')
            EW_removal_efficiency_csv_df.to_csv('C:/PyCharm/PycharmProjects/Grid-based MONET/Output/EW/China/Removal Efficiency/Removal Efficiency_1.csv')

            EW_weathering_rate_csv_df = pd.DataFrame(EW_weathering_rate_csv)
            # EW_weathering_rate_csv_df.to_csv('./Output/EW/Removal Efficiency/Global Output/Weathering Rate.csv')
            EW_weathering_rate_csv_df.to_csv('C:/PyCharm/PycharmProjects/Grid-based MONET/Output/EW/China/Removal Efficiency/Weathering Rate_1.csv')
    else:
