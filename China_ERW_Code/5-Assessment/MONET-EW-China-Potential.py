...
def compute_removal_potential(
    ...
    # 记录每个网格的累积潜力
    EW_rem_potential_csv = np.full((360*720, 8), np.nan)  # MtCO2
    EW_rem_potential_csv_1 = np.full((360*720, 8), np.nan)  # tCO2/ha
    ...
    for r in range(num_of_type):
        ...
        for s in range(num_of_grain_size):
            ...
            if pathway == 2:
                CO2_seq_potential_rock = gdal.Open(f'C:/PyCharm/PycharmProjects/NETs/EW/Output/Raster Data/CO2_seq_potential_{rock_type.iloc[r, 0]}(tCO2·t rock-1)_{grain_size.iloc[s, 0]} μm_1.tif').ReadAsArray()
            # transport distance
                            ...
                            if not np.isnan(EW_global_transport_distance_array_rock[row, col]):  # no rock source in this country
                                if EEF_array[row, col] >= 0:
                                    ...
                                    if CO2_removal_potential_rock > 0:  # tCO2/t rock
                                        ...
                                        EW_potential_raster_array_rock_cap[row, col] = CO2_seq_potential_rock[row, col] * EW_data.iloc[15, 2] * Grid_Cropland_Area_km2[row, col] * 100 * T_TO_GT * 1000  # 用MtCO2为单位
                                        EW_potential_raster_array_rock_rem[row, col] = CO2_removal_potential_rock * EW_data.iloc[15, 2] * Grid_Cropland_Area_km2[row, col] * 100 * T_TO_GT * 1000  # 用MtCO2为单位
                                        EW_rem_potential_csv[row * 720 + col, r * 4 + s] = EW_potential_raster_array_rock_rem[row, col]
                                        EW_rem_potential_csv_1[row * 720 + col, r * 4 + s] = CO2_removal_potential_rock * EW_data.iloc[15, 2]  # tCO2/ha
                                    else:
                                        EW_potential_raster_array_rock_cap[row, col] = np.nan
                                        EW_potential_raster_array_rock_rem[row, col] = np.nan
                                        EW_rem_potential_csv[row * 720 + col, r * 4 + s] = np.nan
                                        EW_rem_potential_csv_1[row * 720 + col, r * 4 + s] = np.nan
            ...
            if uncertainty == False:
                if pathway == 1:
                    ...
                    EW_potential_raster_rock_rem = driver.Create(f'C:/PyCharm/PycharmProjects/Grid-based MONET/Output/EW/China/Remove Potential/EW_remove_potential_raster_{rock_type.iloc[r, 0]}_{grain_size.iloc[s, 0]} μm.tif', xsize=raster_col, ysize=raster_row, bands=1, eType=gdal.GDT_Float32)  # note: xsize=ecological_zones_raster_array.shape[1], not [0]
                ...
                if pathway == 2:
                    ...
                    driver = gdal.GetDriverByName("GTiff")
                    # note: xsize=ecological_zones_raster_array.shape[1], not [0]; eType=gdal.GDT_Float32
                    EW_potential_raster_rock_rem = driver.Create(f'C:/PyCharm/PycharmProjects/Grid-based MONET/Output/EW/China/Remove Potential/EW_remove_potential_raster_{rock_type.iloc[r, 0]}_{grain_size.iloc[s, 0]} μm_1.tif', xsize=raster_col, ysize=raster_row, bands=1, eType=gdal.GDT_Float32)  # note: xsize=ecological_zones_raster_array.shape[1], not [0]
                    EW_potential_raster_rock_rem.SetGeoTransform([-180, 0.5, 0, 90, 0, -0.5])  # 设置地理变换参数;原点；左上角坐标(-180, 90)，像元大小(0.5 x 0.5)，即0.5°分辨率
                    srs = osr.SpatialReference()  # 设置坐标参考系为 WGS 1984 (EPSG:4326)
                    srs.ImportFromEPSG(4326)  # WGS 1984 的 EPSG 编号是 4326
                    EW_potential_raster_rock_rem.SetProjection(srs.ExportToWkt())
    ...
    if uncertainty == False:
        if pathway == 1:
            ...
            EW_rem_potential_csv_df.to_csv('C:/PyCharm/PycharmProjects/Grid-based MONET/Output/EW/China/Remove Potential/Remove Potential.csv', index=False)
            EW_rem_potential_csv_df_1 = pd.DataFrame(EW_rem_potential_csv_1)
            EW_rem_potential_csv_df_1.to_csv('C:/PyCharm/PycharmProjects/Grid-based MONET/Output/EW/China/Remove Potential/Remove Potential (tCO2 ha-1).csv', index=False)
        if pathway == 2:
            # 保存为csv文件
            EW_rem_potential_csv_df = pd.DataFrame(EW_rem_potential_csv)
            EW_rem_potential_csv_df.to_csv('C:/PyCharm/PycharmProjects/Grid-based MONET/Output/EW/China/Remove Potential/Remove Potential_1.csv', index=False)
            EW_rem_potential_csv_df_1 = pd.DataFrame(EW_rem_potential_csv_1)
            EW_rem_potential_csv_df_1.to_csv('C:/PyCharm/PycharmProjects/Grid-based MONET/Output/EW/China/Remove Potential/Remove Potential (tCO2 ha-1)_1.csv', index=False)
    else:
